...
/** Struct to carry MaxSub data. More efficient in divide and conquer than allocating and deleting small dynamic arrays **/
struct node {
    int max;
    int first;
    int last;
...
** Return: 		struct containing node.max (sum), node.first (first index) , node.last (last index) of the maximum subarray
** Description:	Divide and Conquer Maximum Subarray algorithm.
**					Divides problem by 2 until arrays of size 1 is reached. Conquers by rebuilding and finding the max sum of the
**					1) left subarray 2) right subarray 3) cross subarray and returning it.
**					If a new maximum sum is found, it is saved, along with the start/stop indices in the larger array
**				The final max sum, start/stop indices are returned in a struct node
**********************************************************************************************/
